        ...
        $scope.authValid = function(operation) {
            ...
            for (; i < security.length; i++) {
                ...
                for (key in sec) {
                    ...
                    valid.push(auth.valid ? auth.valid : false);
                    auth.scopes =  sec[key];
                    operation.authParams.push(auth);
                }
                return _.includes(valid, true);
            }
            return false;
        };
...
angular
    ...
    .service('swaggerClient', ["$q", "$http", "swaggerModules", function($q, $http, swaggerModules) {
        ...
        this.send = function(swagger, operation, values) {
            ...
            // authorization
            var params = operation.authParams;
            if (params) {
                _.forEach(params, function(authParams){
